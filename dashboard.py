...
st.markdown('<p class="subtitle">Predict energy savings for Malaysian residential buildings using Random Forest</p>', unsafe_allow_html=True)
...
# ==========================================
with tab4:
    st.subheader("📋 Detailed Energy Report")
    ...
    report_data = pd.DataFrame({
        'Parameter': ['Temperature', 'Humidity', 'Hour', 'Day', 'Month', 'Floor Area', 'Occupants', 'Retrofit'],
        'Value': [f"{temp}°C", f"{humidity}%", f"{hour}:00", ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][dayofweek], 
                  ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][month-1], f"{floor_area} m²", f"{occupants}", 'Yes' if retrofit else 'No']
    })
